...
import { deleteCountry, fetchCountries, updateCountry } from './service'
...
const CountriesPage: React.FC = () => {
  ...
  const load = () => {
    setLoading(true)
    fetchCountries()
      .then((r) => setCountries(r.data?.data || r.data?.items || r.data || []))
  ...
  const handleToggle = async (row: any) => {
    try {
      await updateCountry(row._id, { ...row, isActive: !row.isActive })
      toast.success(`Country ${row.isActive ? 'deactivated' : 'activated'}`)
      load()
    } catch {
      ...
      },
      {
        ...
        render: (r: any) => (
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => navigate(`/countries/${r._id}/edit`)} className="btn-secondary btn-sm"><Pencil className="h-3 w-3" /></button>
  ...
  return (
    <div className="page">
      <PageHeader
        ...
        action={<button className="btn-primary" onClick={() => navigate('/countries/new')}><Plus className="h-4 w-4" /> New Country</button>}
      />
      ...
      <ConfirmDialog
        ...
        message="All states connected to this country could break."
        confirmLabel="Delete"
        loading={deleting}
      />
    </div>
